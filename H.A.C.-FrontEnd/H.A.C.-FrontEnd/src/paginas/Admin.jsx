...
const Admin = () => {
  ...
  const handleAtualizarProdutos = async () => {
    try {
      const prodData = await produtosServico.listarProdutos();
      setProdutos(prodData);
    } catch (error) {
        ...
        <main className="col-lg-9 d-flex flex-column gap-5">
                          ...
                          <div>
                            ...
                            <div className="small text-white opacity-75">
                              {p.itens && p.itens.length > 0 ? (
                                p.itens.length === 1 
                                  ? p.itens[0].nome 
                                  : `${p.itens[0].nome} + ${p.itens.length - 1} item(s)`
                              ) : 'Sem itens'}
                            </div>
                            <div className="small text-muted" style={{ fontSize: '0.7rem' }}>{p.cliente?.nome || 'Cliente Arena'}</div>
                          </div>
          ...
          {secaoAtiva === 'produtos' && (
            <div className="animate-in">
              <TabelaProdutosAdmin 
                produtos={produtos} 
                onAtualizar={handleAtualizarProdutos} 
              />
            </div>
          )}

...
const listarPedidos = async () => {
  ...
  if (CONFIG.USE_MOCKS || CONFIG.USE_MOCKS_PEDIDOS) {
...
const buscarPedidoPorId = async (id) => {
  ...
  if (CONFIG.USE_MOCKS || CONFIG.USE_MOCKS_PEDIDOS) {
    ...
    return pedidosCorrigidos.find(p => String(p.id) === String(id));
  }
  ...
  try {
    ...
    const msg = error.response?.data?.mensagem || error.response?.data?.message || error.message || 'Erro ao buscar pedido.';
    throw new Error(msg);
  }
};
...
const criarPedidoAPartirDoCarrinho = async (dadosCheckout, itensCarrinho, usuario) => {
  ...
  try {
    ...
    const msg = error.response?.data?.mensagem || error.response?.data?.message || error.message || 'Erro ao criar pedido.';
    throw new Error(msg);
  }
};

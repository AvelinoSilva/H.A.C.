...
const inicializarUsuariosMock = () => {
  if (!localStorage.getItem(STORAGE_KEYS_MOCK.LISTA_USUARIOS)) {
    localStorage.setItem(STORAGE_KEYS_MOCK.LISTA_USUARIOS, JSON.stringify(USUARIOS_MOCK));
  }
};
...
const login = async (email, senha) => {
  ...
  if (CONFIG.USE_MOCKS || CONFIG.USE_MOCKS_AUTH) {
    ...
    const usuarios = JSON.parse(localStorage.getItem(STORAGE_KEYS_MOCK.LISTA_USUARIOS));
    ...
    if (usuario) {
      ...
      localStorage.setItem(CONFIG.STORAGE_KEYS.USUARIO_LOGADO, JSON.stringify(usuarioSeguro));
  ...
  try {
    ...
    const msg = error.response?.data?.mensagem || error.response?.data?.message || error.message || 'Erro ao realizar login.';
    throw new Error(msg);
  }
};
...
const cadastrar = async (dadosUsuario) => {
  ...
  if (CONFIG.USE_MOCKS || CONFIG.USE_MOCKS_AUTH) {
    ...
    localStorage.setItem(STORAGE_KEYS_MOCK.LISTA_USUARIOS, JSON.stringify(usuarios));
  ...
  try {
    ...
    const msg = error.response?.data?.mensagem || error.response?.data?.message || error.message || 'Erro ao realizar cadastro.';
    throw new Error(msg);
  }
};
...
const obterUsuarioAtual = () => {
  const usuarioJson = localStorage.getItem(CONFIG.STORAGE_KEYS.USUARIO_LOGADO);

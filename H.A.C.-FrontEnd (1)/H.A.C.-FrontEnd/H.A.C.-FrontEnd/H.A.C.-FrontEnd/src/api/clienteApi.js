...
clienteApi.interceptors.response.use(
  ...
  (error) => {
    ...
    if (response) {
      ...
      const mensagem = response.data?.mensagem || response.data?.message || 'Erro no servidor';
      console.error(`[API Error] Status: ${response.status}`, mensagem);
      
      if (response.status === 401 && !error.config?.url?.includes('/auth/login')) {
        // Redirecionar apenas se não for tentativa de login falha
